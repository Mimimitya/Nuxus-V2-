...
wss.on('connection', (ws, req) => {
  ...
  ws.on('message', (data) => {
    try {
      ...
      switch (msg.type) {
        ...
        case 'assign_server_role': handleAssignServerRole(ws, msg); break;
  ...
  function handleCreateServerChannel(ws, msg) {
    ...
    const group = db.groups[server.groupId];
    if (!group || !group.admins.includes(currentUserId)) return;
    
    let category = server.categories.find(c => c.id === msg.categoryId);
    if (!category) {
      // Создаем категорию если не существует
      category = {
        id: msg.categoryId || 'cat_' + crypto.randomBytes(6).toString('hex'),
        name: msg.categoryName || 'General',
        channels: []
      };
      server.categories.push(category);
    }
    
    const channelId = 'ch_' + crypto.randomBytes(6).toString('hex');
    const newChannel = {
      id: channelId,
    ...
    // Создаем чат для канала
    const fullChannelId = `${msg.serverId}_${channelId}`;
    db.messages[fullChannelId] = [];
    db.unreadCounts[fullChannelId] = {};
  ...
  function handleCreateServerRole(ws, msg) {
    ...
    if (server.roles.length >= 5) {
      ws.send(JSON.stringify({ type: 'error', error: 'Maximum 5 roles allowed' }));
      return;
    }
  ...
  function handleAssignServerRole(ws, msg) {
    const server = db.servers[msg.serverId];
    if (!server) return;
    ...
    if (!server.memberRoles[msg.userId]) server.memberRoles[msg.userId] = [];
    
    if (!server.memberRoles[msg.userId].includes(msg.roleId)) {
      server.memberRoles[msg.userId].push(msg.roleId);
      saveDB();
  ...
  function handleBotCommand(ws, msg) {
    ...
    // Robot Builder commands
    if (msg.botId === 'robot_builder_bot' || (user.developerMode && !msg.botId)) {
      const chatId = createChat('robot_builder_bot', currentUserId);
      let responseText = '';
      ...
      switch (command) {
        ...
        case '/mybots':
          ...
          responseText = userBots.length === 0 ? 
            'У вас нет ботов. /newbot [название]' : 
            '🤖 Ваши боты:\n\n' + userBots.map((b, i) => `${i + 1}. ${b.name}\nID: ${b.id}`).join('\n\n');
          break;
          
        case '/help':
          responseText = `📚 Команды Robot Builder:\n\n/newbot [название] - создать бота\n/mybots - список ботов\n/help - справка`;
          break;
    ...
    if (msg.botId === 'sticker_bot' || command.startsWith('/newpack') || command.startsWith('/mypacks') || command.startsWith('/addsticker')) {
      const chatId = createChat('sticker_bot', currentUserId);
      let responseText = '';
      
      switch (command) {
        case '/newpack':
          const packName = args.join(' ') || 'My Sticker Pack';
          const packId = 'pack_' + crypto.randomBytes(8).toString('hex');
          ...
          if (!db.userStickers[currentUserId]) db.userStickers[currentUserId] = ['default'];
          if (!db.userStickers[currentUserId].includes(packId)) {
            db.userStickers[currentUserId].push(packId);
          }
          
          saveDB();
          
          responseText = `✅ Стикерпак создан!\n\n📦 ${packName}\n🆔 ${packId}\n\nТеперь добавьте стикеры:\n/addsticker ${packId} 😀`;
          break;
        ...
        case '/addsticker':
          ...
          if (!targetPackId || !sticker) {
            responseText = '❌ Использование: /addsticker [pack_id] [стикер]\nПример: /addsticker pack_xxx 😀';
            break;
          }
          
          const pack = db.stickerPacks[targetPackId];
          if (!pack) {
            responseText = '❌ Стикерпак не найден';
            break;
          }
          
          if (pack.creator !== currentUserId) {
            responseText = '❌ Это не ваш стикерпак';
            break;
          }
          
          if (!pack.stickers.includes(sticker)) {
            pack.stickers.push(sticker);
            saveDB();
            responseText = `✅ Стикер ${sticker} добавлен в ${pack.name}!\n\nВсего стикеров: ${pack.stickers.length}\n\nДобавить еще:\n/addsticker ${targetPackId} [стикер]`;
          } else {
            responseText = '⚠️ Этот стикер уже в паке';
          }
          break;
          
        case '/mypacks':
          const userPacks = Object.values(db.stickerPacks).filter(p => p.creator === currentUserId);
          if (userPacks.length === 0) {
            responseText = 'У вас нет стикерпаков.\n\n📦 Создать:\n/newpack [название]';
          } else {
            responseText = '📦 Ваши стикерпаки:\n\n' + userPacks.map(p => 
              `${p.name}\nID: ${p.id}\nСтикеров: ${p.stickers.length}\n${p.stickers.slice(0, 10).join(' ')}`
            ).join('\n\n');
          }
          break;
          
        case '/help':
          responseText = `🎨 Команды Sticker Bot:\n\n/newpack [название] - создать стикерпак\n/addsticker [pack_id] [стикер] - добавить стикер\n/mypacks - мои стикерпаки\n/help - справка\n\n💡 Пример:\n/newpack Мои эмодзи\n/addsticker pack_xxx 😀`;
          break;
      ...
      // Отправляем обновленный список стикерпаков
      ws.send(JSON.stringify({
        type: 'sticker_packs_updated',
        stickerPacks: db.stickerPacks,
        userStickers: db.userStickers[currentUserId] || ['default']
      }));
    }
  }
});
